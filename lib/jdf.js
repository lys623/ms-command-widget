...
jdf.init = function(argv) {
    ...
    jdf.getConfig(function(configData) {
        ...
        if (argv.length < 3 || cmd2 === '-h' || cmd2 === '--help') {
            ...
            var cmd3 = argv[3];
            Log.send('server');
            Server.init('./', cmd3||jdf.config.localServerPort, jdf.config.cdn, jdf.getProjectPath(), true);
            console.log('jdf server running at http://localhost:' + cmd3||jdf.config.localServerPort + '/');
