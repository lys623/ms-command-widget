...
var options=global.settings.connect
//建立链接
...
ftp.downloadMain = function(source, target, callback){
	//console.log('downloadMains:'+source);
	ftp.listMain(source, function(data){
		//console.log('listMain:'+data)
		if (data != 'error') {
			f.mkdir(target);
			var serverNum=0, localNum=0;
...
ftp.listMain = function(source, callback){
	//console.log('listMainSource:'+source)
	ftp.client.list(source, function(err, list) {
		//console.log('listMain:'+JSON.stringify(list))
		if (err){
...
ftp.mkdir = function(source, callback){
	...
	ftp.client.on('ready',function(){
		//ftp.client.put('/da0/index.html', 'foo.remote-copy.txt', function(err) {
		//	if (err) throw err;
		//	ftp.client.end();
		//});
		ftp.client.mkdir(source , true, function(err){
